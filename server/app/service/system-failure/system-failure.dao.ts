...
import {
    SystemFailureAttribute,
    SystemFailureInstance,
    SystemFailureDefinitions
} from './system-failure.model';
...
interface SystemFailureDao {
    getSystemFailure(
        ...
        limit?: number
    ): Promise<Array<SystemFailureInstance>>;

    saveSystemFailure(
        systemFailure: Array<SystemFailureAttribute>
    ): Promise<Array<SystemFailureInstance>>;
}
...
class SystemFailureDaoImpl extends BaseDao<SystemFailureInstance, SystemFailureAttribute> implements SystemFailureDao {
    ...
    public getSystemFailure(system?: string, begin?: number, end?: number, limit?: number): Promise<Array<SystemFailureInstance>> {
        ...
        if (system !== undefined)
            searchParam.where.system = system;

        if (begin !== undefined || end !== undefined) {
            searchParam.where.time = { };

            if (begin !== undefined)
                searchParam.where.time.$gte = begin;

            if (end !== undefined)
                searchParam.where.time.$lte = end;
        }
